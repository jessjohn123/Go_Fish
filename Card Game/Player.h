...
private:
	...
	int m_numCards = 0;				// The number of cards the player is currently holding

	/* TODO Lab5:
			Update m_maxCards to be a const data member
			*/
	const int m_maxCards = 0;				// The number of cards the player can store (the number of elements in Hand)
	int m_score = 0;				// For "Go Fish," this will represent the number of pairs.  For "UNO," it will be the player's accumulated score
...
public:
	...
	/* TODO Lab2:
			...
			*/
	Player(const char* _name = {"Player"}, int _maxCards = 14);
	//Player(const char* _name, int _maxCards);
	...
	*/
	...
	static bool GetCheat(int ch_option);

	static void SetCheat(int option);
